...
import { monaTraceabilityApi, type RawTraceabilityRow } from '../api/mona-traceability-api';
...
export const monaTraceabilityRepository: TraceabilityRepository = {
  async lookup(query: TraceabilityQuery): Promise<TraceabilitySnapshot> {
    ...
    const res = await monaTraceabilityApi.getStatus(query.id);
