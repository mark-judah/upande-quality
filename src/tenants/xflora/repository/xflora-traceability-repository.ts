...
import { xfloraTraceabilityApi, type RawTraceabilityRow } from '../api/xflora-traceability-api';
...
export const xfloraTraceabilityRepository: TraceabilityRepository = {
  async lookup(query: TraceabilityQuery): Promise<TraceabilitySnapshot> {
    ...
    const res = await xfloraTraceabilityApi.getStatus(query.id);
