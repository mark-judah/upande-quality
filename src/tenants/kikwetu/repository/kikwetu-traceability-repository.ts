...
import { kikwetuTraceabilityApi, type RawTraceabilityRow } from '../api/kikwetu-traceability-api';
...
export const kikwetuTraceabilityRepository: TraceabilityRepository = {
  async lookup(query: TraceabilityQuery): Promise<TraceabilitySnapshot> {
    ...
    const res = await kikwetuTraceabilityApi.getStatus(query.id);
